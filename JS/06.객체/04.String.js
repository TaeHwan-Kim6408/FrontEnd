...
console.log(hello.indexOf('세요'), hello.indexOf('여러분!'));                    // 3, -1
console.log(hello.includex('세요'));
console.log('pineapple'.lastIndexOf('p'));                                       // 6

// 문자열을 분리하여 배열을 생성
let fruits = '수박,참외,포도';
let fruitArry = fruits.split(',');
console.log(fruitArry);                                                          // ['수박', '참외', '포도']
...
console.log(sample.replace('o', '0'));

//정규표현식(Regular Expression)
sample = 'A quick brown fox - 3 - 갈색의 재빠른 여우 the lazy dog = 5 = 게으른 개';
// 영문자와 공백만 남기고 모두 지움
console.log(ssample.replace(/[^A-Z a-z]/g, ''));
// 한글과 공백만 남기고 모두 지움
console.log(ssample.replace(/[^가-힣ㄱ-ㅎ ㅏ-ㅣ]/g, ''));